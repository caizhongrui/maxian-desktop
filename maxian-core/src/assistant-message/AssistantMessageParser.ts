...
// Copied from Kilocode src/core/assistant-message/AssistantMessageParser.ts
// Complete implementation

import { type ToolName, toolNames, TextContent, ToolUse, ToolParamName, toolParamNames } from '../types/toolTypes.js';
import { AssistantMessageContent } from './parseAssistantMessage.js';
import { extractMcpToolInfo, NativeToolCall, parseDoubleEncodedParams } from './NativeToolCall.js';

// Type for Anthropic tool use block params (simplified version for now)
...
export class AssistantMessageParser {
	 ...
	 */
	public *processNativeToolCalls(toolCalls: NativeToolCall[]): Generator<AnthropicToolUseBlockParam> {
		for (const toolCall of toolCalls) {
			...
			if (isComplete) {
				...
				if (mcpToolInfo) {
					...
					normalizedToolName = 'use_mcp_tool' as ToolName;
